...
class LanceDBMemoryProvider(_MemoryProviderBase):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            self._ready = True
            logger.info("LanceDB memory v1.6.0 initialized at %s", self._storage_path)
    ...
    def _hybrid_search(self, query: str, top_k: int = _TOP_K_RETURN) -> List[Dict]:
        ...
        try:
            ...
            # then de-duplicate by id, preserving best rank per id.
            try:
            ...
            try:
                for q in expanded_queries[:3]:  # cap to avoid latency blow-up
                    try:
                        rows = (
                            self._table.search(q, query_type="fts")
                            .where(f"user_id = '{self._user_id}'", prefilter=True)
                            .limit(_TOP_K_BM25)
                            .to_list()
                        )
    ...
    def _write_entries(self, entries: List[Dict]) -> None:
        ...
        try:
            ...
            for entry in entries:
                ...
                if not text.strip():
                    continue
                # P4: compute temporal_type if not pre-set. Falls back to "static"
                # if the classifier raises (degrades gracefully).
                ...
                rows.append({
                    ...
                    "temporal_type": str(temporal or "static"),
                })
            if rows:
